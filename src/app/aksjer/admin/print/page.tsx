...
export default async function StockAdminPrintPage() {
  ...
  const error = settingsRes.error?.message || companyRes.error?.message || shareholdersRes.error?.message || lotsRes.error?.message || null;
  if (error) {
    redirect(`/aksjer/admin?error=${encodeURIComponent(error)}`);
  }

  const generatedAt = new Date().toLocaleString('nb-NO');
  const companyName = companyRes.data?.company_name || 'AI Innovate AS';
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-gray-500 mt-1">
                Org.nr: {companyRes.data?.orgnr || '-'} • Stiftet: {companyRes.data?.incorporation_date || '-'} • Aksjekapital:{' '}
                {companyRes.data?.share_capital ?? '-'} • Pålydende: {companyRes.data?.par_value ?? '-'}
              </div>
            </div>
