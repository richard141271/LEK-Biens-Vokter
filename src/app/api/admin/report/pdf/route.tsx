...
export async function GET(request: Request) {
  ...
  // --- Render HTML Parts ---
  const scoreCardsHtml = renderToStaticMarkup(
  ...
  const barChartHtml = renderToStaticMarkup(<SimpleBarChart data={[
  ...
  const donutChartHtml = renderToStaticMarkup(<DonutChart data={pieData} total={totalAnswers} />);
  
  const ratingsHtml = type === 'BEEKEEPER' ? renderToStaticMarkup(
  ...
  try {
    ...
    // Set content and wait for Tailwind to load/apply (networkidle0 might be too slow if external CDN, so just wait a bit or use explicit wait)
    // Actually, tailwind CDN is fast, but we should wait for network idle to be safe.
