...
export default function WarRoomPage() {
    ...
    const loadMessages = async () => {
        ...
        if (res.messages) {
            setMessages(res.messages.reverse()); // Show oldest first (at top) or newest first?
            // Chat usually has newest at bottom.
            // API returns newest first (desc).
            // So reversing makes oldest first.
        }
        setLoading(false);
    };
            ...
            <header className="bg-white border-b border-gray-200 py-4 px-6 sticky top-0 z-10 shadow-sm">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href="/dashboard/founder" className="text-gray-500 hover:text-gray-900 text-sm">
                            ← Tilbake
                        </Link>
                ...
                <div className="max-w-4xl mx-auto">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <input
                            ...
                            className="flex-1 rounded-lg border-gray-300 focus:border-amber-500 focus:ring-amber-500 p-3 shadow-sm"
                            disabled={sending}
                        />
                        <button
                            type="submit"
