...
import { ArrowLeft, MapPin, Truck, Calendar, Activity, X, Check } from 'lucide-react';
    ...
    try {
      ...
      const { error: logError } = await supabase
        ...
        .insert({
          ...
          user_id: user?.id,
          action: 'FLYTTET',
          details: `Flyttet fra ${hive.apiaries?.name} til ${targetApiary?.name} (${targetApiary?.apiary_number})`
        });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <main className="p-4 space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-full inline-block text-sm">
                {hive.status}
              </p>
            </div>
          </div>
        ...
        {/* History / Logs */}
        <div>
          <h3 className="text-lg font-bold text-gray-900 mb-3">Historikk</h3>
          <div className="space-y-3">
            {logs.length === 0 ? (
              <p className="text-gray-500 text-center py-4">Ingen historikk enda.</p>
            ) : (
              logs.map((log) => (
